...
/* ─── Contenido reutilizable (desktop + mobile) ────────────────────────── */

function SidebarContent({ collapsed, onItemClick }: SidebarBaseProps) {
...
/* ─── Sidebar para desktop (lg+): visible siempre, colapsable ──────────── */
  ...
  return (
    <TooltipProvider delayDuration={300}>
      <motion.aside
        ...
        className={cn(
          'hidden lg:flex h-screen flex-col overflow-hidden shrink-0',
...
/* ─── MobileSidebar: drawer Sheet en <lg ───────────────────────────────── */
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        ...
        className="w-[280px] sm:w-[300px] p-0 lg:hidden"
      >
        <SheetTitle className="sr-only">Menú de navegación</SheetTitle>
        <SheetDescription className="sr-only">
          Navega entre las secciones de HRCO
        </SheetDescription>
        <TooltipProvider delayDuration={300}>
          <div className="flex h-full flex-col bg-white dark:bg-ink-900/95">
...
function NavItem({ to, label, Icon, collapsed, onClick }: NavItemProps) {
  const link = (
    <NavLink
      to={to}
      onClick={onClick}
      className={({ isActive }) =>
        cn(
          'group relative flex items-center gap-3 h-11 lg:h-10 rounded-xl text-sm font-medium transition-colors',
